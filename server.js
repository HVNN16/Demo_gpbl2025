...
let latestSensorData = {}; // Lưu dữ liệu mới nhất thay vì array lớn
let ledState = 0; // Trạng thái LED (0: Tắt, 1: Bật)

// API điều khiển LED
app.get("/toggle-led", (req, res) => res.json({ state: ledState }));

app.post("/toggle-led", (req, res) => {
    const { state } = req.body;
    if (state === undefined) return res.status(400).send({ success: false, message: "Thiếu trạng thái LED!" });
...
// Nhận dữ liệu cảm biến từ Arduino
app.post("/sensor-data", (req, res) => {
    latestSensorData = { ...req.body, timestamp: new Date() };
    console.log("🔥 Dữ liệu cảm biến cập nhật:", latestSensorData);

    io.emit("updateSensorData", latestSensorData);
    res.send("Dữ liệu đã nhận!");
});

// API lấy dữ liệu cảm biến mới nhất
app.get("/sensor-data", (req, res) => res.json(latestSensorData));

// WebSocket kết nối
io.on("connection", (socket) => {
    console.log("✅ Client đã kết nối WebSocket");
    socket.emit("updateSensorData", latestSensorData);
    socket.on("disconnect", () => console.log("❌ Client đã ngắt kết nối WebSocket"));
});
